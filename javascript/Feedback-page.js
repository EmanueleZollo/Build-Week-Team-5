...
// CONTROLLO SE ALMENO UNA STELLA É STATA SELEZIONATA E IL TESTO É STATO MESSO

const checkConditions = function () {
  const starsSelected = singleStar > 0;
  const inputValid = inputRequired.value.length > 10;

  if (starsSelected && inputValid) {
    ...
    websiteButton.setAttribute("disabled", "true");
    websiteButton.classList.remove("submitButton");
    websiteButton.classList.add("inactiveSubmitButton");
  }
};

inputRequired.addEventListener("input", checkConditions);
checkConditions();
